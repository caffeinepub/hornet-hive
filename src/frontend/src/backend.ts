...
export class Backend implements backendInterface {
    ...
    async setUniqueUsername(arg0: string): Promise<void> {
        if (this.processError) {
            try {
                const result = await this.actor.setUniqueUsername(arg0);
                return result;
            } catch (e) {
                this.processError(e);
                throw new Error("unreachable");
...
function from_candid_record_n14(_uploadFile: (file: ExternalBlob) => Promise<Uint8Array>, _downloadFile: (file: Uint8Array) => Promise<ExternalBlob>, value: {
    ...
    return {
        name: value.name,
        accountSuspendedUntil: record_opt_to_undefined(from_candid_opt_n15(_uploadFile, _downloadFile, value.accountSuspendedUntil))
    };
}
...
function to_candid_record_n24(_uploadFile: (file: ExternalBlob) => Promise<Uint8Array>, _downloadFile: (file: Uint8Array) => Promise<ExternalBlob>, value: {
    ...
    return {
        name: value.name,
        accountSuspendedUntil: value.accountSuspendedUntil ? candid_some(value.accountSuspendedUntil) : candid_none()
    };
}
