...
export default function WeeklyPollPanel({
  compact = false,
}: WeeklyPollPanelProps) {
  const {
    poll,
    pollState,
    selectedOption,
  ...
  const isSubmitDisabled =
    !selectedOption || (selectedOption === "other" && !customResponse.trim());
              ...
              {postOptions.length === 0 ? (
                <div className="text-center py-6 text-muted-foreground">
                  ...
                  <p className="text-sm mt-2">
                    You can still vote using the "Other" option below.
                  </p>
                </div>
              ) : null}
              ...
              {allOptions.length === 0 ? (
                ...
                allOptions
                  .map((option) => ({
                    ...
                    percentage:
                      totalVotes > 0
                        ? ((poll.votes[option.id] || 0) / totalVotes) * 100
                        : 0,
                  }))
                  ...
                  .map(({ id, label, votes, percentage, isCustom }) => (
                    <div key={id} className="space-y-2">
                      <div className="flex items-start justify-between gap-2">
                        <span
                          className={`text-sm font-medium ${isCustom ? "italic" : ""}`}
                        >
                          {label}
                          {poll.userVote === id && (
